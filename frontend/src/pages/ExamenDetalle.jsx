...
export default function ExamenDetalle() {
  ...
  const exportarPDF = async () => {
    ...
    doc.setFontSize(11)
    doc.setFont('helvetica', 'bold')
    doc.text("ACTA DE EXAMEN", 172.5, 23, { align: 'center' })

    doc.setTextColor(40)
    doc.setFontSize(10)
    doc.setFont('helvetica', 'normal')
    doc.text(`Examen:`, 145, 32)
    doc.setFont('helvetica', 'bold')
    doc.text((examen?.nombre || 'Examen').toUpperCase(), 145, 37)
    doc.setFont('helvetica', 'normal')
    doc.text(`Fecha: ${formatearFechaNatural(examen?.fecha)}`, 145, 43)

    // TABLA
    const tableColumn = ['#', 'Alumno', 'Grado Actual', 'Grado Aspirado', 'Costo', 'Estado Pago', 'Resultado']
    ...
    doc.roundedRect(125, summaryY, 75, 14, 2, 2, 'FD')

    doc.setFontSize(10)
    doc.setTextColor(30, 41, 59)
    doc.setFont('helvetica', 'bold')
    doc.text('Total Recaudado:', 130, summaryY + 9)
    doc.setTextColor(16, 185, 129)
    doc.setFontSize(12)
    doc.text(`$${Math.round(totalIngresos)}`, 195, summaryY + 9, { align: 'right' })
